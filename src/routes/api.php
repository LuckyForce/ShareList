...
//TODO: Change all DB::select or DB::insert or DB::update to DB::table
...
Route::post('/register', function (Request $request) {
    ...
    $user = DB::table('sl_u_user')->where('u_email', $request->email)->where('u_emailverified', 1)->first();
    if ($user) {
        return response()->json(['message' => 'User already exists'], 400);
    }

    //Check if user already exists but not already verified
    $user = DB::table('sl_u_user')->where('u_email', $request->email)->where('u_emailverified', 0)->first();
    if ($user) {
        //User already exists. Overwrite Password.
        DB::table('sl_u_user')->where('u_email', $request->email)->update(['u_password' => password_hash($request->password, PASSWORD_DEFAULT)]);
    }else{
        //Create new user
        DB::table('sl_u_user')->insert(['u_email' => $request->email, 'u_password' => password_hash($request->password, PASSWORD_DEFAULT)]);
    }
...
Route::post('/verify', function (Request $request) {
    ...
    //Get the user from the database
    $user = DB::table('sl_u_user')->where('u_email', $request->email)->where('u_emailcode', $request->code)->first();
    ...
    //Update the user
    DB::table('sl_u_user')->where('u_id', $user->u_id)->update(['u_emailcode' => null, 'u_emailverified' => 1]);
